...
def book(id):
    ...
    no_of_comment = cur.execute('SELECT * FROM comments WHERE bookid = %s ORDER BY comment_date',[id])
    ...
    for j in range(0,len(comment)):
        ...
        single_comment = comment[j]
        user_id = single_comment['userid']
        cur.execute('SELECT * FROM users WHERE id = "%s"', [user_id])
        user_info = cur.fetchone()
        single_comment.update({'username':user_info['name']})
    ...
    return render_template('book.html', book=book, comment_list = comment_list, no_of_comment=no_of_comment)
...
def category(category,page_no,item_per_page):
    ...
    if category == 'books':
        cur.execute("SELECT COUNT(*) FROM books ORDER BY import_date DESC")
    else:
        cur.execute('SELECT COUNT(*) FROM books WHERE category = "%s" ORDER BY import_date DESC' %category)
    ...
    if category == 'books':
        cur.execute('SELECT * FROM books ORDER BY import_date DESC')
    else:
        cur.execute('SELECT * FROM books WHERE category = "%s" ORDER BY import_date DESC' %category)
    ...
    if no_of_books > 0:
        ...
        return render_template('category.html', item_per_page = item_per_page,header=header,category = category,no_of_pages=no_of_pages,all_book_info=all_book_info,page_no=page_no,show_list=show_list)
    else:
...
def add_order(category,item_per_page,page_no,bookid):
    ...
    if category == 'no' and page_no == 'no' and item_per_page == 'no' :
        ...
        return redirect(url_for('category',category=category,item_per_page=item_per_page,page_no=page_no))
...
def search():
	if request.method == 'POST':
		...
		if found == 0:
			...
			return render_template('new_arrivals.html', error=error, key_word = key_word)
		else:
			return render_template('search.html', result=result, key_word = key_word)
...
def add_comment():
    if request.method == 'POST':
        ...
        # Get the comment, bookid from the form
        ...
        if comment != '':
            ...
            cur.execute("INSERT INTO comments (bookid,content,userid) VALUES(%s, %s, %s)", (book_id,comment,user_id))
            mysql.connection.commit()
        ...
        if comment!='':
            ...
            flash('Please enter comment before sumbission','danger')
            return redirect(url_for('book',id=book_id))
